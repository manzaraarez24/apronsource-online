...
const Navbar = ({ cartCount, onCartClick, searchQuery, onSearchChange }: NavbarProps) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Add floating glassmorphism if scrolled past 20px
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 px-0 sm:px-4 py-0 sm:py-3 transition-transform duration-500 ease-in-out pointer-events-none" style={{ transform: isHidden ? "translateY(-100%)" : "translateY(0)" }}>
      <nav
        className={`pointer-events-auto mx-auto max-w-7xl transition-all duration-300 ease-in-out ${isScrolled
          ? "glass-strong border-glow shadow-2xl shadow-primary/5 sm:rounded-2xl"
